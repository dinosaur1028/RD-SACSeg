...
from nets.unet_ori import Unet as unet
...
class Unet(object):
    _defaults = {
        ...
        #-------------------------------------------------------------------#
        "model_path"    : '/dataset/zhuluoji/unet/unet_origin/train_weight_output/2025-06-21 14:13:58/best_epoch_weights.pth',
        #--------------------------------#
        #   所需要区分的类的个数+1
        #--------------------------------#
        "num_classes"   : 2,
    ...
    def detect_image(self, image, count=False, name_classes=None):
        ...
        return image
    def detect_image2(self, image, count=False, name_classes=None):
        #---------------------------------------------------------#
        #   在这里将图像转换成RGB图像，防止灰度图在预测时报错。
        #   代码仅仅支持RGB图像的预测，所有其它类型的图像都会转化成RGB
        #---------------------------------------------------------#
        ...
        if self.mix_type == 0:
            ...
            image   = Image.fromarray(np.uint8(seg_img))
            # 1. 转为numpy数组
            original_np = np.array(old_img)
            mask_np = np.array(image.convert('L'))  # 转灰度
            ...
            # 4. 在原图上绘制轮廓
            # 注意opencv用BGR，PIL用RGB，颜色需转换
            contour_color_bgr = (255, 0, 0)
            image_with_contour = original_np.copy()
            cv2.drawContours(image_with_contour, contours, -1, contour_color_bgr, 1)
