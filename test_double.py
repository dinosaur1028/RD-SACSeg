...
def predict_single_image(unet, img_path, out_r1_dir, out_r2_dir):
    ...
    """
    # 构造 dex 和 lab 图片路径（根据实际情况修改替换规则）
    ...
    if not os.path.exists(dex_path):
        print(f"dex图片不存在：{dex_path}")
        return
    if not os.path.exists(lab_path):
        print(f"lab图片不存在：{lab_path}")
        return
...
def batch_predict(unet, input_dir, output_root):
    ...
    jpeg_dir = os.path.join(input_dir, "JPEGImages")
...
def main():
    ...
    input_folder = "/dataset/zhuluoji/unet/unet_double/VOCdevkit3/VOC2007"  # 根目录下包含 JPEGImages、mask_output_folder、LAB_dir
