...
# net_name = 'unet_double_fuse_bandpass_exchange'hua
# from nets.unet_double_fuse_bandpass_exchange import Unet
# from nets.unet_double_fuse_bandpass import Unet
net_name = 'unet'
...
if __name__ == "__main__":
    ...
    save_dir            = f'train_weight_output/{t}'
    if not os.path.exists("train_weight_output"):
        os.mkdir("train_weight_output")
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    net_path = f'{parent_directory}/nets/{net_name}.py'
    trainpy_path = f'{parent_directory}/train.py'
    shutil.copy2(net_path, save_dir)
    shutil.copy2(trainpy_path, save_dir)
    ...
    # VOCdevkit_path  = '/dataset/zhuluoji/unet/unet_dexined/VOCdevkit'
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    param_logger = ParameterLogger(log_dir=f'./logs/{t}', model=model)
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
    ...
    #   读取数据集对应的txt
    #---------------------------#
    with open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Segmentation/train.txt"),"r",encoding="gbk") as f:
        train_lines = f.readlines()
    with open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Segmentation/val.txt"),"r",encoding="gbk") as f:
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            param_logger.log_parameters(epoch)
        param_logger.save_final_curves()
        if local_rank == 0:
